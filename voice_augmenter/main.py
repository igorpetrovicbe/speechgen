...
def modify_audio(input_path, output_path, speed_factor, pitch_shift_factor, bit_rate="47k"):
    # Convert MP3 to WAV
    ...
    for filename in os.listdir(input_folder):
        if filename.endswith(".mp3"):
            ...
            for i in range(num_variations):
                # Randomly select speed factor and pitch shift factor
                speed_factor = np.random.uniform(0.9, 1.1)
                pitch_shift_factor = np.random.uniform(0.5, 1.5)  # pitch shift factor
                ...
                # Modify audio
                modify_audio(input_path, output_path, speed_factor, pitch_shift_factor, bit_rate)


# Example usage
input_folder = "H:\\PycharmProjects\\vqvae-audiotrack\\srpski_train_dataset"
